...
export default function SubscriptionSection({ subscription }: SubscriptionSectionProps) {
  ...
  const handleSubscribe = (plan: typeof plans[0]) => {
    // ── Not logged in → save redirect destination, open signup ──
    if (!isLoggedIn()) {
      const redirectUrl = `/payment?plan=${encodeURIComponent(plan.name)}&amount=${plan.price}&duration=${encodeURIComponent(plan.duration)}`;
    ...
    if (subscription?.status === "pending") {
      alert("Your subscription is pending approval. Please wait.");
      return;
    }
  ...
  const getSubscriptionDisplay = () => {
    ...
    if (subscription.status === "pending") {
      return {
        text: "Pending", color: "#fff7e6", status: "Pending Approval",
        planName: subscription.planName || "Basic Plan", expiryDate: "N/A", daysRemaining: 0,
      };
    }
  ...
  return (
    <div style={{ padding: 32, position: "relative", zIndex: 1 }}>
      ...
      {!loggedIn && (
        <div style={{
          background: "#fff3ea", border: "1.5px solid #ffd666", borderRadius: 12,
          padding: 16, marginBottom: 24, display: "flex", alignItems: "center",
          gap: 12, flexWrap: "wrap"
        }}>
          <span style={{ fontSize: 24 }}>🔒</span>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, color: "#d46b08", marginBottom: 2 }}>Sign up to subscribe</div>
            <div style={{ fontSize: 13, color: "#ad6800" }}>Create a free account to access subscription plans</div>
          </div>
          <button
            onClick={() => { window.location.href = "/?auth=signup"; }}
            style={{ background: "#924DAC", color: "#fff", border: "none", borderRadius: 8, padding: "8px 20px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}
          >
            Sign Up Free →
          </button>
        </div>
      ...
      {subscription && (
        <div style={{
          background: subDisplay.color, borderRadius: 12, padding: 24, marginBottom: 32,
          boxShadow: "0 2px 12px rgba(146,77,172,0.08)",
          border: subscription.status === "pending" ? "2px solid #ffd666" : "none",
        }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: "#924DAC", marginBottom: 12 }}>Your Current Plan</h3>
      ...
      {/* Available Plans */}
      <h3 style={{ fontSize: 18, fontWeight: 700, color: "#924DAC", marginBottom: 16 }}>Available Plans</h3>
        ...
        {plans.map((plan) => {
          ...
          return (
            <div key={plan.name} style={{
              background: plan.color, borderRadius: 12,
              boxShadow: isCurrentPlan ? "0 0 0 2px #924DAC, 0 2px 8px rgba(146,77,172,0.1)" : "0 2px 8px rgba(146,77,172,0.04)",
              padding: 28, minWidth: 220, flex: "1 1 220px", position: "relative", zIndex: 2
            }}>
              {isCurrentPlan && (
              ...
              <button
                ...
                style={{
                  ...
                  background: isCurrentPlan ? "#ccc" : !loggedIn ? "#f0a500" : "#924DAC",
              ...
              >
                {isCurrentPlan ? "Active Plan" : !loggedIn ? "🔒 Sign Up to Subscribe" : "Subscribe"}
              </button>
            </div>
          );
        })}
