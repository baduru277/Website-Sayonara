  ...
  return (
    <>
      <style>{`
        ...
        .activity-toast { animation: toastIn 0.45s cubic-bezier(.17,.67,.35,1.2) forwards; }
      `}</style>
      <div className="activity-toast" style={{
        ...
        maxWidth: 310, minWidth: 250,
      }}>
        <div style={{ width: 44, height: 44, borderRadius: "50%", background: current.color, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14, flexShrink: 0 }}>{current.avatar}</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#1a0533" }}>{current.name} <span style={{ color: "#888", fontWeight: 500 }}>from {current.city}</span></div>
          <div style={{ fontSize: 12, color: "#555", marginTop: 3 }}>{current.action} <strong style={{ color: current.color }}>{current.item}</strong>{current.price && <span style={{ color: "#16a34a", fontWeight: 700 }}> {current.price}</span>}</div>
        </div>
        <button onClick={() => setVisible(false)} style={{ background: "none", border: "none", color: "#ccc", cursor: "pointer", fontSize: 18, padding: 0 }}>×</button>
        <div style={{ position: "absolute", bottom: 0, left: 0, height: 3, background: current.color, borderRadius: "0 0 18px 18px", animation: "progressShrink 4.5s linear forwards" }} />
      </div>
    </>
  );
}
