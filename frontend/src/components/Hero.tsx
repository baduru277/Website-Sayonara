    ...
    <>
      <section style={{
        ...
        <div style={{
          ...
          textAlign: 'center', gap: 28, paddingTop: 64, paddingBottom: 64,
        }}>
          ...
          <form onSubmit={handleSearchSubmit} style={{ width: '100%', maxWidth: 480, position: 'relative' }}>
            <input
              ...
              style={{ width: '100%', padding: '16px 120px 16px 24px', border: '2px solid #924DAC', borderRadius: 999, fontSize: 17, color: '#924DAC', fontWeight: 500, outline: 'none', boxSizing: 'border-box' }}
            />
            <button type="button" onClick={() => {
              setReqItem(search.trim());
              if (!loggedIn) { localStorage.setItem('pendingItemRequest', search.trim()); window.location.href = '/?auth=signup'; return; }
              setShowRequestForm(true);
            }} style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', background: '#924DAC', color: '#fff', border: 'none', borderRadius: 999, padding: '9px 20px', fontWeight: 700, fontSize: 14, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              Request →
            </button>
          </form>
          ...
          <button onClick={handleGetStarted} className="sayonara-btn" style={{
            fontSize: 20, marginTop: 12, padding: '12px 36px', border: 'none', cursor: 'pointer',
          ...
          >
            ...
            <div style={{ flex: 1, textAlign: 'left' }}>
              <div style={{ color: '#fff', fontWeight: 800, fontSize: 15, marginBottom: 3 }}>
                Refer a Friend & Earn FREE Premium!
              </div>
              <div style={{ color: 'rgba(255,255,255,0.82)', fontSize: 12, lineHeight: 1.4 }}>
                Both you & your friend get <strong style={{ color: '#f9d423' }}>1 month FREE</strong> — worth ₹99!
              </div>
            </div>
              ...
              <>
                ...
                <form onSubmit={handleRequestSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div>
                    ...
                    <input type="text" value={reqBudget} onChange={e => setReqBudget(e.target.value)} placeholder="e.g. ₹5000 - ₹10000" style={{ width: '100%', padding: '10px 14px', border: '1.5px solid #e0d0f0', borderRadius: 8, fontSize: 15, outline: 'none', boxSizing: 'border-box' }} />
                  </div>
