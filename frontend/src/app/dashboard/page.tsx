  ...
  const renderContent = () => {
    switch (selected) {
          ...
          <div style={{ padding: 32 }}>
            ...
            <div style={{ 
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20, marginBottom: 24 }}>
                {["📦 Declutter & Earn", "🤝 Build Community", "🌱 Go Green"].map((text, i) => (
                  <div key={i} style={{ 
                    background: "white", 
                    padding: 20, 
                    borderRadius: 12, 
                    ...
                    <span style={{ fontSize: 32, marginBottom: 12, display: "block" }}>{text.split(' ')[0]}</span>
                    <h4 style={{ color: "#924DAC", fontWeight: 600, marginBottom: 8 }}>{text.split(' ').slice(1).join(' ')}</h4>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20, marginTop: 32 }}>
              {["Items Posted","Total Views","Total Likes","Successful Trades"].map((label, i) => (
                <div key={i} style={{ background: "white", padding: 24, borderRadius: 12, textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
            ...
            <table style={{ width: "100%", background: "#fff", borderRadius: 12, boxShadow: "0 2px 12px rgba(146,77,172,0.06)", overflow: "hidden" }}>
              ...
              <tbody>
                {userProfile?.items && userProfile.items.length > 0 ? (
                  userProfile.items.map((item: any) => (
                    <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                      <td style={{ padding: 12, fontWeight: 500 }}>{item.title || "Deleted by user"}</td>
                      <td style={{ padding: 12 }}>
                        <span style={{
                          padding: "4px 8px",
                          borderRadius: "4px",
                  ...
                  <tr>
                    <td colSpan={6} style={{ padding: 20, textAlign: "center", color: "#888" }}>
                      No items have been posted yet. Your deleted items will appear as "Deleted by user".
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      case "subscription":
        return (
          <div style={{ padding: 32 }}>
            <h2 style={{ color: "#924DAC", fontWeight: 700, fontSize: 22, marginBottom: 18 }}>Subscription</h2>
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
              {["Free Plan","Silver Plan","Gold Plan"].map((plan, i) => (
                <div key={i} style={{ background: "white", padding: 20, borderRadius: 12, boxShadow: "0 2px 12px rgba(146,77,172,0.06)", flex: "1 1 200px", textAlign: "center" }}>
                  <h3 style={{ color: "#924DAC", fontWeight: 600, marginBottom: 12 }}>{plan}</h3>
                  <p style={{ color: "#666", fontSize: 14, marginBottom: 12 }}>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
                  <button className="sayonara-btn" style={{ padding: "8px 20px", fontSize: 14 }}>Subscribe</button>
                </div>
              ))}
            </div>
          </div>
        ...
        return (
          <div style={{ padding: 32 }}>
            ...
            <div style={{ background: "white", borderRadius: 12, padding: 24, boxShadow: "0 2px 12px rgba(146,77,172,0.06)" }}>
              <h3 style={{ color: "#924DAC", fontWeight: 600, marginBottom: 16 }}>Profile</h3>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <input type="text" placeholder="First Name" defaultValue={userProfile?.firstName || ""} style={{ padding: 12, borderRadius: 6, border: "1px solid #ccc" }} />
                <input type="text" placeholder="Last Name" defaultValue={userProfile?.lastName || ""} style={{ padding: 12, borderRadius: 6, border: "1px solid #ccc" }} />
                <input type="email" placeholder="Email" defaultValue={userProfile?.email || ""} style={{ padding: 12, borderRadius: 6, border: "1px solid #ccc" }} />
                <input type="text" placeholder="Phone Number" defaultValue={userProfile?.phone || ""} style={{ padding: 12, borderRadius: 6, border: "1px solid #ccc" }} />
              </div>
              <button className="sayonara-btn" style={{ marginTop: 20, padding: "10px 24px", fontSize: 14 }}>Update Profile</button>
            </div>
          </div>
        );
