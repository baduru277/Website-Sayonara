...
// Define types for the mocked Link component props
...
// Mock useRouter hook
const useRouter = () => ({ 
  push: (path: string) => console.log('Simulated Redirect to Onboarding:', path) 
});
...
const apiService = {
  ...
  register: async (userData: any) => {
    console.log('API: Attempting registration with data for:', userData.email);
...
type FormErrors = Record<keyof FormData | 'general', string>;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.token) {
        ...
        // Redirect to the Onboarding/Basic Information step
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#7F53AC] to-[#647DEE] font-inter py-8">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Location (Indian States/UTs) */}
          <div>
            <label className="block font-semibold text-gray-700 text-sm mb-2">State / Union Territory *</label>
            <select
              name="location"
