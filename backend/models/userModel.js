...
const userSchema = new mongoose.Schema({
  ...
  activityLogs: [
    {
      ...
      activityType: {
        type: String,
        enum: ['login', 'posting', 'transaction', 'other'], 
        required: true,
      },
  ...
  idDocument: {
    type: String, // Can be a file path, URL, or base64-encoded string
    required: true,
  },
  bankStatement: {
    type: String, // Can be a file path, URL, or base64-encoded string
...
// Add Activity Logs
userSchema.methods.addActivityLog = function (activityType = null) {
  if (activityType) {
    return this.activityLogs.filter((log) => log.activityType === activityType);
  }
